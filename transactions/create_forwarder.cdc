...
transaction(receiver: Address) {
    ...
    prepare(acct: AuthAccount) {
        let recipient = getAccount(receiver)
            .getCapability<&{FungibleToken.Receiver}>(/public/liliumReceiver)

        let vault <- TokenForwarding.createNewForwarder(recipient: recipient)
        acct.save(<-vault, to: /storage/liliumForwarder)

        if acct.getCapability(/public/liliumReceiver).check<&{FungibleToken.Receiver}>() {
            acct.unlink(/public/liliumReceiver)
        }

        acct.link<&{FungibleToken.Receiver}>(
            /public/liliumReceiver,
            target: /storage/liliumForwarder
        )
    }
}
